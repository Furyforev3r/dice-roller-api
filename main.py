...
async def main(dice_input: str):
    ...
    for dice_verificacao in dice_split:
        try:
            dices_mod.append(int(dice_verificacao)) 
        except ValueError:
    ...
    for dices_verificacao in range(0, len(dices)):
        try:
            ...
            try:
                dice_mod = [dices_mod[dices_verificacao]]
            ...
            dices_dict[dices_verificacao] = {'Amount': dices_amount, 'Sides': dices_sides, 'Results': dices_array, 'Mod': dice_mod, 'ModSum': mod_sum, 'TotalNoMod': dice_sum, 'TotalMod': dice_sum+mod_sum, 'Greater': greater, 'Less': less, 'Average': greater/less, 'GreaterWMod': greater+mod_sum}
        except ValueError:
